...
.main,section.main{{padding:0 !important;overflow:hidden !important;height:100vh !important;}}

/* Scrollable content between fixed header and footer */
.block-container{{
...
/* other widgets */
.stButton>button{{
    background:rgba(255,255,255,0.08) !important;color:{T["muted"]} !important;
...
# ══════════════════════════════════════════════
# MOBILE HEADER — injected via JS
# This is the version that was working — pills are SMALL,
# placed to the RIGHT of title, do NOT overlap it
# ══════════════════════════════════════════════
...
if is_mobile:
    ...
    # Mode pill config — small icon + text, pill shape, NOT overlapping
    pill_configs = [
        ("General",  "💬", T["accent"],  "rgba(59,158,255,0.25)",  "rgba(59,158,255,0.6)"),
        ("Document", "📄", T["accent3"], "rgba(0,201,167,0.25)",   "rgba(0,201,167,0.6)"),
        ("URL",      "🔗", T["accent2"], "rgba(0,212,255,0.25)",   "rgba(0,212,255,0.6)"),
    ]

    pills_html = ""
    for m, icon, col, bg, bord in pill_configs:
        active   = current_mode == m
        pill_bg  = col if active else bg
        pill_brd = col if active else bord
        glow     = f"0 0 10px {col}90" if active else "none"
        txt_col  = "#ffffff"
        pills_html += (
            f'<a href="?mode={m}&mobile=1" style="'
            f'background:{pill_bg};'
            f'border:1.5px solid {pill_brd};'
...
(function(){{
  try{{
    ...
    hdr.id='srin-hdr';
    hdr.style.cssText='position:fixed;top:0;left:0;right:0;height:68px;'
    ...
    hdr.innerHTML=`
      ...
      <a href="?mode={current_mode}&mobile=1&theme={theme_link}"
        style="font-size:20px;text-decoration:none;flex-shrink:0;margin-left:8px;
          display:flex;align-items:center;justify-content:center;
