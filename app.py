...
defaults = {
    ...
    "device": "desktop", "fs": False, "last_active": time.time(),
}
...
# CSS
# ══════════════════════════════════════════════
FONT = "@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500&display=swap');"
...
[data-testid="stChatMessage"] p{{color:{T["text"]} !important;font-size:14px !important;
...
# The key fix: target EVERY possible stBottom child element
BOTTOM_CSS = f"""
...
[data-testid="stFileUploader"]{{background:{T["card"]} !important;border:1.5px dashed {T["border2"]} !important;border-radius:10px !important;}}
...
iframe[height="0"]{{position:absolute !important;width:0 !important;height:0 !important;border:none !important;pointer-events:none !important;}}
"""
...
.stApp{{background:{T["chat_bg"]} !important;font-family:'Outfit',sans-serif !important;
    height:100vh !important;overflow:hidden !important;width:100vw !important;}}
.main,section.main{{padding:0 !important;overflow:hidden !important;
    height:100vh !important;width:100vw !important;background:{T["chat_bg"]} !important;}}
.block-container{{
    position:fixed !important;
...
{MSG_CSS}{BOTTOM_CSS}{WIDGET_CSS}
</style>""", unsafe_allow_html=True)

# ─────────────── TABLET ───────────────
elif is_tablet:
    # KEY FIX: Use JS to set the parent body background
    # CSS inside Streamlit cannot reach the parent document
    # We inject gradient via JS on parent body + a centred panel
    st.markdown(f"""<style>
{FONT}
*,*::before,*::after{{box-sizing:border-box;margin:0;padding:0;}}
{HIDE_CHROME}
html,body{{height:100vh;overflow:hidden;}}
/* Make stApp transparent so JS-injected parent bg shows through */
.stApp{{
    background:transparent !important;
    font-family:'Outfit',sans-serif !important;
...
/* Centred chat panel — dark navy, floating on gradient sides */
.block-container{{
    position:fixed !important;
    top:68px !important;
    bottom:68px !important;
    ...
    box-shadow:-50px 0 100px rgba(0,0,0,0.8),
               ...
               0 0 0 1px rgba(59,158,255,0.12) !important;
    z-index:10 !important;
}}
...
[data-testid="stChatInput"] textarea {{
    ...
    width:100% !important;min-height:50px !important;max-height:50px !important;flex:1 !important;
}}
[data-testid="stChatInput"] textarea::placeholder {{
...
[data-testid="stChatInput"] button:hover{{transform:scale(1.08) !important;}}
[data-testid="stChatInput"] button svg{{fill:#ffffff !important;stroke:#ffffff !important;width:20px !important;height:20px !important;}}
{MSG_CSS}{WIDGET_CSS}
</style>""", unsafe_allow_html=True)
...
.stApp{{background:linear-gradient(160deg,#040a14 0%,#071020 25%,#091528 55%,#0c1c38 80%,#0e2040 100%) !important;
...
[data-testid="stChatInput"]{{background:#0c1a32 !important;border:1.5px solid {T["border2"]} !important;
    border-radius:18px !important;box-shadow:0 4px 28px rgba(0,0,0,0.4) !important;
...
[data-testid="stChatInput"]:focus-within{{border-color:{T["accent"]} !important;
    box-shadow:0 4px 32px rgba(59,158,255,0.25) !important;}}
...
[data-testid="stChatInput"] button{{background:linear-gradient(135deg,{T["accent"]},{T["accent2"]}) !important;
    border:none !important;border-radius:12px !important;width:38px !important;height:38px !important;
...
# ══════════════════════════════════════════════
# TABLET: inject gradient into PARENT document via JS
# This is the ONLY reliable way to set body background
# from inside a Streamlit iframe
# ══════════════════════════════════════════════
if is_tablet:
    st.components.v1.html(f"""
<script>
(function(){{
  try{{
    // Set parent body/html background to landing page gradient
    var s = window.parent.document.documentElement.style;
    s.background = '{SIDE_GRAD}';
    s.height = '100vh';
    s.overflow = 'hidden';
    var bs = window.parent.document.body.style;
    bs.background = '{SIDE_GRAD}';
    bs.height = '100vh';
    bs.overflow = 'hidden';
    bs.margin = '0';

    // Add dot grid overlay
    var overlay = window.parent.document.getElementById('srin-overlay');
    if(!overlay){{
      overlay = window.parent.document.createElement('div');
      overlay.id = 'srin-overlay';
  ...
  }}catch(e){{console.log('overlay err',e);}}
}})();
</script>
""", height=0, scrolling=False)
...
if is_mobile or is_tablet:
    ...
    hdr_w      = "100%"   if is_mobile else "620px"
    hdr_left   = "0"      if is_mobile else "50%"
    hdr_xform  = "none"   if is_mobile else "translateX(-50%)"
    hdr_mw     = "100vw"  if is_mobile else "92vw"
    show_hdr   = "none" if (is_mobile and is_fs) else "flex"
    ...
    for m in ["General","Document","URL"]:
        ...
        pills_html += (
            f'<a href="?mode={m}&device={device}&fs={1 if is_fs else 0}" style="'
    ...
    if is_mobile:
        ...
        fs_btn = (
            f'<a href="?mode={current_mode}&device=mobile&fs={fs_next}" style="'
            f'background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.2);'
            f'border-radius:999px;padding:4px 10px;font-size:12px;font-weight:700;'
            f'color:#fff;text-decoration:none;white-space:nowrap;margin-left:2px;">'
            f'{fs_icon}{fs_label}</a>'
        )
    else:
        fs_btn = ""
...
(function(){{
  try{{
    ...
    hdr.id='srin-hdr';
    hdr.style.cssText=
      'position:fixed;top:0;left:{hdr_left};transform:{hdr_xform};'
      +'width:{hdr_w};max-width:{hdr_mw};height:68px;'
      +'background:linear-gradient(135deg,{T["card"]},{T["card2"]});'
      +'border-bottom:1px solid {T["border2"]};'
      +'z-index:999999;display:{show_hdr};align-items:center;'
      +'padding:0 14px;'
    ...
    hdr.innerHTML=`
      ...
      <a href="?mode={current_mode}&device={device}&fs={1 if is_fs else 0}&theme={theme_link}"
...
<div style="background:linear-gradient(135deg,{T['card']} 0%,{T['card2']} 100%);
    border:1px solid {T['border2']};border-radius:16px;padding:14px 22px;
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;">
    ...
    with col_t:
        tog = st.toggle("🌙",value=(st.session_state.theme=="Dark"),
                        key="ttog",label_visibility="collapsed")
        ...
        for i,m in enumerate(["General","Document","URL"]):
            with mc[i]:
                ...
                if st.button(f"{'● ' if active else ''}{m_icons[m]} {m}",
                             key=f"mb_{m}", use_container_width=True):
                    if current_mode != m:
...
if current_mode=="Document":
    st.markdown(f"""<div style="background:{T['card']};border:1px solid {T['border2']};
        border-left:3px solid {T['accent3']};border-radius:{br};padding:{pad};margin-bottom:8px;">
      <div style="font-size:{fs1};color:{T['accent3']};font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:3px;">
        📄 Upload Document</div>
    ...
    if uploaded and uploaded.name!=st.session_state.doc_name:
        ...
        try:
            ...
            if text.strip():
                ...
                st.session_state.doc_context=text[:mc2]; st.session_state.doc_name=uploaded.name
    ...
    st.markdown(f"""<div style="background:{T['card']};border:1px solid {T['border2']};
        border-left:3px solid {T['accent2']};border-radius:{br};padding:{pad};margin-bottom:8px;">
      <div style="font-size:{fs1};color:{T['accent2']};font-weight:700;letter-spacing:1px;text-transform:uppercase;margin-bottom:3px;">
        🔗 Load a URL</div>
    ...
    if load_btn and url_val:
        with st.spinner("Fetching..."):
            try:
                ...
                clean="\n".join(lines); mc2=6000 if is_mobile else 12000
                st.session_state.url_context=clean[:mc2]; st.session_state.url_loaded=url_val
...
if not st.session_state.messages and not doc_missing and not url_missing:
    ...
    st.markdown(f"""<div style="text-align:center;padding:{'20px 10px' if is_mobile else '44px 20px'};color:{T['muted']};">
      <div style="font-size:{'36px' if is_mobile else '44px'};margin-bottom:10px;">{ico}</div>
      <div style="font-size:{'15px' if is_mobile else '17px'};font-weight:700;color:{T['text']};margin-bottom:8px;">{title}</div>
      <div style="font-size:{'11px' if is_mobile else '12px'};color:{T['muted']};font-style:italic;
        background:{T['card']};border:1px solid {T['border']};border-radius:8px;
        padding:6px 14px;display:inline-block;">{hint}</div>
    </div>""", unsafe_allow_html=True)
...
def build_prompt(user_input):
    mob=("IMPORTANT: User is on mobile. Max 120 words. Bullet points.\n\n") if is_mobile else ""
...
# ══════════════════════════════════════════════
if is_desktop:
    status={"General":"💬 General AI",
            "Document":f"📄 {st.session_state.doc_name}" if st.session_state.doc_name else "📄 No document",
            "URL":f"🔗 {(st.session_state.url_loaded or '')[:45]}" if st.session_state.url_loaded else "🔗 No URL"}[current_mode]
    ...
    st.markdown(f"""<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);
      ...
      <div style="font-size:10px;color:{T['muted']};display:flex;align-items:center;gap:8px;">
        ...
        <span style="color:{'#ff8800' if ip>70 else T['muted']};">⏱ {int((IDLE_TIMEOUT-ie)/60)}m left</span>
      </div>
    </div>""", unsafe_allow_html=True)
